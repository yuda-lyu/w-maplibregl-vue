<template>
    <div ref="root" style="display:inline-block; position:relative;" v-domresize @domresize="resize">
        ...
        <div v-for="corner in corners" :key="corner" class="clsCorner" :class="'clsCorner-'+corner">
                ...
                <div :style="{ overflow:'auto', ...panelLabels.style }">
                    ...
                    <table><tbody>
                        <tr v-for="(u, ku) in panelLabels.useItems" :key="ku">
                            ...
                            <td>{{ u==='lng'||u==='lat' ? showLoc[u] : u==='zoom' ? Math.round(currentZoom*1e5)/1e5 : '' }}</td>
                        </tr>
                    </tbody></table>
                </div>
            </div>
            ...
            <!-- 圖例區（等值線用） -->
            <div v-if="panelLegends.show && countVisible(contourSets)>0 && panelLegends.position===corner" class="clsPanel" :style="{ background: panelBackgroundColor }">
    ...
    methods: {
        ...
        initMap() {
            ...
            vo.map = new maplibregl.Map({
                ...
                style: { version: 8, sources: {}, glyphs: 'https://demotiles.maplibre.org/font/{fontstack}/{range}.pbf', layers: [{ id: 'background', type: 'background', paint: { 'background-color': '#f0f0f0' } }] },
        ...
        changeOpt() {
            ...
            if (clusterChanged && vo.map && vo.map.isStyleLoaded()) {
        ...
        changeContourSets() {
            ...
            if (vo.map && vo.map.isStyleLoaded()) vo.renderContourSets()
        },
